...
export async function getPublicStore(slug) {
  ...
  const { data } = await client.from("stores").select("id,name,slug,seller_code,seller_code_active,order_access_suspended,description,whatsapp,phone,address,logo_url,brand_color,is_published,approval_status,rejection_reason").eq("slug", slug).in("approval_status", ["approved", "pending"]).maybeSingle();
  return data;
}
