...
export function getPublicHomeSamples() {
  return unstable_cache(async () => {
    ...
    const [{ data: stores }, { data: products }] = await Promise.all([
      client.from("stores").select("id,name,slug,category,logo_url,brand_color,paid_verification_approved").eq("is_published", true).eq("approval_status", "approved").eq("paid_verification_approved", true).order("created_at", { ascending: false }).limit(6),
