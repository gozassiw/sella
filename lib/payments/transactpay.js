...
function getConfig() {
  ...
  if (!publicKey || !secretKey || !encryptionKey) {
    throw new Error("TransactPay public, secret, and encryption keys are not configured");
  }
...
export function providerConfigured() {
  return Boolean(process.env.TRANSACTPAY_PUBLIC_KEY && process.env.TRANSACTPAY_SECRET_KEY && (process.env.TRANSACTPAY_ENCRYPTION_KEY || process.env.TRANSACTPAY_PUBLIC_KEY));
}
