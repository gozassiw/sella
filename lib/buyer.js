...
export const getBuyerProfile = cache(async (userId) => {
  if (!userId) return null;
  const supabase = createClient();
  const { data } = await supabase.from("buyer_profiles").select("full_name").eq("user_id", userId).maybeSingle();
  return data || null;
});

export const getBuyerWallet = cache(async (userId) => {
  if (!userId) return null;
  const supabase = createClient();
  const { data } = await supabase.from("buyer_wallets").select("*").eq("user_id", userId).maybeSingle();
  if (data) return data;
  const { data: created, error } = await supabase.rpc("get_or_create_buyer_wallet");
