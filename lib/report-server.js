...
export function validateReportTarget(body) {
  ...
  if (type !== "order" && !isUuid(storeId)) return { error: "A valid store is required." };
...
export async function validateBuyerTarget(auth, user, { storeId, orderId, productId, type }) {
  ...
  if (type === "order" && orderId) {
    const { data: order } = await auth.from("orders").select("id,store_id,buyer_id,order_number,total").eq("id", orderId).eq("buyer_id", user.id).maybeSingle();
    if (!order || !isUuid(order.store_id)) return { error: "That order is not linked to your account or a valid store." };
