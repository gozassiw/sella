...
export function CancelOrderForm({ order, orderId, onDone }) {
  const paid = order?.payment_status === "paid";
  return <ActionForm action="cancel" onDone={onDone}>
    <input type="hidden" name="orderId" value={order?.id || orderId} />
    <label className="label">{paid ? "Cancel this paid order?" : "Cancellation reason"}</label>
    {paid && <div className="rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-900"><p>This customer has already paid. Cancelling will create a refund of {Number(order.total || 0).toLocaleString("en-NG", { style: "currency", currency: "NGN" })} for the buyer.</p><p className="mt-2">Order #{order.order_code} · Buyer: {order.customers?.name || "Customer"} · Store: {order.stores?.name || "Your store"}</p><p>Original payment / refund: {Number(order.total || 0).toLocaleString("en-NG", { style: "currency", currency: "NGN" })}</p><p>Seller available balance is checked securely before any wallet credit.</p></div>}
    <input className="input" name="reason" placeholder="Why are you cancelling this order?" required />
    <button className="btn-danger" disabled={false}>{paid ? "Confirm cancellation" : "Cancel and restock"}</button>
  </ActionForm>;
}
