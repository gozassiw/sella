...
export default function OrderStatusForm({ order }) {
  ...
  return <div className="flex flex-col items-end gap-2">
    ...
    {!['shipped', 'delivered', 'cancelled'].includes(status) && <button type="button" disabled={saving} onClick={cancelOrder} className="text-[11px] font-bold text-danger underline underline-offset-4 disabled:opacity-50">Cancel order</button>}
