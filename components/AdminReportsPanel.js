    ...
    <button type="button" onClick={() => onSelect(report.id)} className={`w-full border-b border-line p-4 text-left transition last:border-0 hover:bg-surface ${selected ? "bg-kola-light/50" : "bg-white"}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 truncate text-xs text-muted">{report.stores?.name || "Store"}{report.orders?.order_number ? ` · Order #${report.orders.order_number}` : ""}</p>
        </div>
...
export default function AdminReportsPanel({ initialReports = [] }) {
  ...
  async function loadDetail(reportId) {
    ...
    const sequence = ++requestVersion.current;
    setSelectedId(reportId);
    setDetail(null); setNote(""); setStatusMessage("");
    setMessage("");
    try {
      ...
      if (sequence !== requestVersion.current) return;
      setDetail(data);
  ...
  async function performAction(action, actionStatus = null) {
    if (!selectedId || detail?.report?.id !== selectedId || busy) return;
    ...
    try {
      const response = await fetch("/api/reports/admin", {
        ...
        body: JSON.stringify({ reportId: selectedId, action, status: actionStatus, body: action === "status" ? statusMessage : note, sellerVisible }),
      });
      const data = await response.json().catch(() => ({}));
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(250px,340px)_1fr]">
         ...
         <div className="app-card min-w-0 p-5 sm:p-6 xl:sticky xl:top-24 xl:self-start">
          {!detail?.report ? <div className="flex min-h-64 flex-col items-center justify-center text-center"><Flag size={26} className="text-muted" /><p className="mt-3 font-semibold">Select a case to review</p><p className="mt-1 text-sm text-muted">Case evidence and private notes appear here.</p></div> : <>
            ...
            <div className="mt-5 border-t border-line pt-5"><label className="text-xs font-bold uppercase tracking-wide text-muted" htmlFor="case-status">Update status</label><label className="mt-3 block text-xs text-muted">Status message (visible to buyer)<textarea className="input mt-1" rows={2} maxLength={5000} value={statusMessage} onChange={(event) => setStatusMessage(event.target.value)} placeholder="Optional buyer-facing progress update" /></label><div className="mt-2 flex flex-wrap gap-2"><select id="case-status" className="input max-w-xs py-2.5 text-xs" value={status} onChange={(event) => setStatus(event.target.value)}>{REPORT_STATUSES.map((item) => <option key={item} value={item}>{item}</option>)}</select><button type="button" className="btn-primary inline-flex items-center gap-2 px-4 py-2 text-xs" disabled={busy || !status} onClick={() => performAction("status", status)}><RefreshCw size={14} />Save status</button></div></div>
            <div className="mt-5 border-t border-line pt-5"><label className="text-xs font-bold uppercase tracking-wide text-muted" htmlFor="case-note">Private note, action record or message to seller</label><textarea id="case-note" className="input mt-2" rows={4} maxLength={5000} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Private note / Record action keeps this text internal. Contact seller sends this text to the seller. Never include passwords or payment PINs." /><div className="mt-3 flex flex-wrap items-center gap-2"><button type="button" className="btn-soft inline-flex items-center gap-2 px-3 py-2 text-xs" disabled={busy} onClick={() => performAction("internal_note")}><MessageSquare size={14} />Private note</button><button type="button" className="btn-secondary inline-flex items-center gap-2 px-3 py-2 text-xs" disabled={busy} onClick={() => performAction("admin_action")}><ShieldCheck size={14} />Record action</button><button type="button" className="btn-primary inline-flex items-center gap-2 px-3 py-2 text-xs" disabled={busy} onClick={() => performAction("seller_request")}><Send size={14} />Contact seller</button><label className="ml-1 inline-flex items-center gap-2 text-xs text-muted"><input type="checkbox" checked={sellerVisible} onChange={(event) => setSellerVisible(event.target.checked)} />Also share the status message with the seller</label></div></div>
