...
export default function PaidVerificationForm({ storeId, price, balance, currentStatus, purchase }) {
  ...
  if (currentStatus === "approved") return <div className="rounded-2xl bg-blue-50 p-5 text-[#1D4ED8]"><div className="flex items-center gap-2 font-extrabold"><BadgeCheck size={21} fill="currentColor" />Verification checkmark active</div><p className="mt-2 text-sm leading-6">Your store has been reviewed and approved by Sella Team. It is eligible for the verified-store discovery feed until the six-month renewal date.</p></div>;
