...
export default function PaidVerificationForm({ storeId, price, balance, currentStatus, purchase }) {
  ...
  if (currentStatus === "approved") return <div className="rounded-2xl bg-blue-50 p-5 text-[#1D4ED8]"><div className="flex items-center gap-2 font-extrabold"><BadgeCheck size={21} fill="currentColor" />Blue checkmark active</div><p className="mt-2 text-sm leading-6">Your store has been reviewed and approved by Sella Team. It is eligible for the verified-store discovery feed.</p></div>;
  ...
  return <div><div className="mb-3 grid gap-2"><label className={`flex cursor-pointer items-start gap-3 rounded-xl border p-3 ${paymentMethod === "wallet" ? "border-kola bg-kola-light" : "border-line bg-white"}`}><input type="radio" name="verification-payment" value="wallet" checked={paymentMethod === "wallet"} onChange={() => setPaymentMethod("wallet")} className="mt-1" /><span><span className="flex items-center gap-2 text-sm font-bold"><WalletCards size={16} />Pay from Sella balance</span><span className="mt-1 block text-xs text-muted">Available: {formatNaira(balance)}</span></span></label><label className={`flex cursor-pointer items-start gap-3 rounded-xl border p-3 ${paymentMethod === "transfer" ? "border-kola bg-kola-light" : "border-line bg-white"}`}><input type="radio" name="verification-payment" value="transfer" checked={paymentMethod === "transfer"} onChange={() => setPaymentMethod("transfer")} className="mt-1" /><span><span className="block text-sm font-bold">Bank transfer</span><span className="mt-1 block text-xs text-muted">One-time account active for 30 minutes.</span></span></label></div><button type="button" className="btn-primary w-full" onClick={submit} disabled={busy || (paymentMethod === "wallet" && Number(balance) < Number(price))}>{busy ? "Processing…" : "Pay for review"}</button>{message && <p className="mt-2 text-xs font-bold text-danger">{message}</p>}</div>;
}
