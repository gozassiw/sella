...
export default function DashboardNav({ store }) {
  ...
  return <>
    ...
    <header className="sticky top-0 z-30 flex items-center justify-between border-b border-line bg-white/95 px-4 py-3.5 backdrop-blur md:hidden"><SellaBrand href="/dashboard" /><div className="flex items-center gap-2"><NotificationBell><Link href="/dashboard/notifications" aria-label="Notifications" className="grid h-10 w-10 place-items-center rounded-xl bg-surface text-kola"><Bell size={17} /></Link></NotificationBell><span className="rounded-xl bg-kola-light px-3 py-2 text-xs font-extrabold tracking-wider text-kola">{store.seller_code || "…"}</span></div></header>

    {moreOpen && <div className="fixed inset-x-3 bottom-[86px] z-50 mx-auto max-h-[70vh] max-w-[520px] overflow-y-auto rounded-[26px] border border-line bg-white p-4" style={{ boxShadow: "var(--shadow-float)" }}><div className="flex items-center justify-between px-1 pb-3"><div><p className="eyebrow text-kola">Workspace</p><p className="mt-1 text-base font-extrabold">More tools</p></div><button onClick={() => setMoreOpen(false)} className="grid h-10 w-10 place-items-center rounded-full bg-surface" aria-label="Close menu"><X size={19} /></button></div><div className="grid grid-cols-2 gap-2">{tools.map((item) => { const Icon = item.icon; const link = <Link key={item.href} href={item.href} onClick={() => setMoreOpen(false)} className={`flex items-center gap-3 rounded-2xl p-3.5 text-xs font-bold ${active(item.href) ? "bg-kola-light text-kola" : "bg-surface text-ink"}`}><Icon size={17} />{item.label}</Link>; return item.href === "/dashboard/notifications" ? <NotificationBell key={item.href}>{link}</NotificationBell> : link; })}</div><button onClick={signOut} className="mt-3 flex w-full items-center justify-center gap-2 rounded-2xl bg-red-50 px-4 py-3 text-xs font-bold text-danger"><LogOut size={17} />Log out</button></div>}
