...
export default function DashboardNav({ store }) {
  ...
  const desktopItem = (item) => { const Icon = item.icon; const selected = active(item.href); return <Link key={item.href} href={item.href} className={`flex items-center gap-3 rounded-2xl px-3.5 py-3 text-sm font-bold transition ${selected ? "bg-mango text-kola-dark" : "text-white/70 hover:bg-white/10 hover:text-white"}`}><Icon size={18} strokeWidth={selected ? 2.3 : 1.8} />{item.label}</Link>; };
  ...
  return <>
    ...
    <header className="sticky top-0 z-30 flex items-center justify-between border-b border-line bg-white/95 px-4 py-3.5 backdrop-blur md:hidden"><SellaBrand href="/dashboard" /><a href={storeUrl(SITE_URL, store.slug)} target="_blank" rel="noreferrer" className="rounded-xl bg-kola-light px-3 py-2 text-xs font-extrabold text-kola">View store</a></header>
    ...
    <nav className="fixed inset-x-3 bottom-3 z-40 mx-auto grid max-w-[520px] grid-cols-5 rounded-[22px] border border-line bg-white/95 p-1.5 backdrop-blur md:hidden" style={{ boxShadow: "var(--shadow-float)" }}>
      {primary.map((item, index) => { const Icon = item.icon; const selected = active(item.href); return <Link key={item.href} href={item.href} className={`flex min-h-[52px] flex-col items-center justify-center gap-1 rounded-2xl text-[10px] font-bold ${index === 2 ? "mx-1 bg-kola text-white" : selected ? "bg-kola-light text-kola" : "text-muted"}`}><Icon size={19} strokeWidth={selected || index === 2 ? 2.3 : 1.8} /><span>{item.label}</span></Link>; })}
