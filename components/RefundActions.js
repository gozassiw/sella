...
export default function RefundActions({ refund }) {
  ...
  return <div className="mt-4 space-y-3 rounded-2xl bg-surface p-4">
    ...
    <div className="flex flex-wrap gap-2"><button type="button" className="btn-primary text-xs" disabled={busy !== "" || refund.refund_status === "refunded"} onClick={processRefund}>{busy === "process" ? "Processing…" : "Process full refund"}</button>{pendingFunding && <button type="button" className="btn-soft inline-flex items-center gap-2 text-xs" disabled={busy !== ""} onClick={requestFundingAccount}>{busy === "fund" ? "Preparing…" : "Fund refund"}</button>}{refund.linked_withdrawal_status && <span className="inline-flex items-center rounded-xl bg-white px-3 py-2 text-[11px] font-bold text-warning">Pending withdrawal: {refund.linked_withdrawal_status}</span>}</div>
    {account && <div className="rounded-2xl border border-kola/20 bg-white p-4 text-xs"><p className="font-extrabold text-kola">Deposit exactly {formatNaira(account.amount)} to this refund account</p><button type="button" onClick={() => copy(account.accountNumber)} className="mt-3 flex w-full items-center justify-between gap-3 rounded-xl bg-kola-light px-3 py-3 text-left text-base font-extrabold text-kola"><span>{account.accountNumber}</span><Copy size={16} /></button><p className="mt-2 text-muted">{account.bankName || "Bank account"}{account.accountName ? ` · ${account.accountName}` : ""}</p><p className="mt-1 text-muted">Reference: {account.reference}. Expires {new Date(account.expiresAt).toLocaleString("en-NG")}.</p><p className="mt-2 leading-5 text-muted">After the deposit is verified, return here and process the full refund. The deposit does not refund the buyer by itself.</p></div>}
