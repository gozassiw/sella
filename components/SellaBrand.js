...
export default function SellaBrand({ href = "/", inverted = false, compact = false }) {
  return (
    <Link href={href} className="inline-flex items-center gap-2.5" aria-label="Sella home">
      <span className={`grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-[11px] ${inverted ? "bg-white" : "bg-surface"}`}>
        <Image src="/brand/sella-mark.png" alt="" width={42} height={42} priority className="h-9 w-9 object-contain" />
      </span>
