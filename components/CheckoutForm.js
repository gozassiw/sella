...
export default function CheckoutForm({ initialProfile = null, paymentSettings = null }) {
  ...
  const bankTransferFeeRate = Number(paymentSettings?.bank_transfer_fee_rate || 0);
  ...
  return <form onSubmit={submit} className="mx-auto max-w-2xl space-y-6">
    ...
    <div className="panel space-y-4"><h2 className="font-semibold">Payment</h2><label className={`block cursor-pointer rounded-xl border p-4 ${form.paymentMethod === "wallet" ? "border-kola bg-kola-light" : "border-line"}`}><input type="radio" className="mr-2" checked={form.paymentMethod === "wallet"} onChange={() => update("paymentMethod", "wallet")} /><strong>Pay from Sella wallet</strong><span className="mt-1 block pl-6 text-sm text-muted">Use your balance across Sella stores.</span></label><label className={`block cursor-pointer rounded-xl border p-4 ${form.paymentMethod === "transfer" ? "border-kola bg-kola-light" : "border-line"}`}><input type="radio" className="mr-2" checked={form.paymentMethod === "transfer"} onChange={() => update("paymentMethod", "transfer")} /><strong>Bank transfer</strong><span className="mt-1 block pl-6 text-sm text-muted">Get a unique account number for this order.</span>{bankTransferFeeRate > 0 && <span className="mt-2 block pl-6 text-xs font-semibold text-kola">A {bankTransferFeeRate}% fee is added to the amount you transfer.</span>}</label></div>
