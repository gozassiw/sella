  ...
  return (
    <nav className="fixed inset-x-3 bottom-3 z-40 mx-auto grid max-w-[520px] grid-cols-5 rounded-[22px] border border-line bg-white/95 p-1.5 backdrop-blur" style={{ boxShadow: "var(--shadow-float)" }}>
      {items.map(({ href, label, icon: Icon, primary }) => {
        ...
        return <Link key={href} href={href} className={`relative flex min-h-[52px] flex-col items-center justify-center gap-1 rounded-2xl text-[10px] font-bold transition ${primary ? "mx-1 bg-kola text-white" : active ? "bg-kola-light text-kola" : "text-muted hover:bg-surface hover:text-ink"}`}><Icon size={19} strokeWidth={active || primary ? 2.3 : 1.8} /><span>{label}</span></Link>;
      })}
    </nav>
  );
}
