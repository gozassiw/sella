...
export default function AuthShell({ title, subtitle, children, footer }) {
  return <div className="min-h-screen bg-white lg:grid lg:grid-cols-[.9fr_1.1fr]">
    <aside className="relative hidden overflow-hidden bg-kola-dark p-12 text-white lg:flex lg:flex-col lg:justify-between">
      <SellaBrand inverted />
      <div className="max-w-md"><span className="chip bg-white/10 text-mango">Commerce, made human</span><h2 className="display mt-6 text-[48px] leading-[1.04]">One account. Every store. Your whole business.</h2><p className="mt-5 text-sm leading-7 text-white/65">Buyers follow products they love. Sellers manage money, stock, orders, and customers without the usual clutter.</p><div className="mt-10 grid gap-3">{[{ icon: ShoppingBag, text: "Discover and follow independent stores" }, { icon: WalletCards, text: "Pay through one reusable buyer wallet" }, { icon: BadgeCheck, text: "Built-in delivery and trust controls" }].map(({ icon: Icon, text }) => <div key={text} className="flex items-center gap-3 rounded-2xl bg-white/10 p-4"><span className="grid h-9 w-9 place-items-center rounded-xl bg-mango text-kola-dark"><Icon size={17} /></span><p className="text-sm font-bold">{text}</p></div>)}</div></div>
