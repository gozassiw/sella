...
export default function AdminWithdrawalActions({ item }) {
  const displayStatus = item.status === "sent" || item.status === "paid" ? "payout completed" : item.status === "processing" ? "payment reference pending" : item.status;
  return (
    <div className="flex flex-wrap items-center justify-end gap-2">
      ...
      {(item.status === "paid" || item.status === "sent") && (item.receipt_path ? <span className="rounded-full bg-kola-light px-2.5 py-1 text-[10px] font-extrabold text-kola">Payout completed · reference uploaded</span> : <WithdrawalReceiptForm withdrawalId={item.id} />)}
    </div>
  );
}
