...
export default function AdminWithdrawalActions({ item }) {
  ...
  return (
    <div className="flex flex-wrap items-center justify-end gap-2">
      ...
      {item.status === "pending" && <>
        <AdminActionForm action="withdrawal_status" field="withdrawalId" value={item.id} nextValue="processing" label="Start processing" />
      ...
      {item.status === "processing" && <>
        ...
        {item.receipt_path ? <AdminActionForm action="withdrawal_status" field="withdrawalId" value={item.id} nextValue="paid" label="Mark paid" /> : <span className="max-w-[180px] text-right text-[11px] font-semibold text-warning">Upload receipt before marking paid</span>}
        <AdminActionForm action="withdrawal_status" field="withdrawalId" value={item.id} nextValue="rejected" label="Cancel" />
      </>}
