    ...
    <>
      ...
      {open && <div className="fixed inset-0 z-[70] h-[100dvh] overflow-y-auto bg-white p-5 md:hidden" role="dialog" aria-modal="true" aria-label="Admin menu">
          <div className="mx-auto w-full max-w-xl">
            ...
            <nav className="mt-5 space-y-2">{items.map(([id, label, Icon]) => { const item = <button key={id} type="button" onClick={() => jumpTo(id)} className="flex min-h-14 w-full items-center gap-4 rounded-xl border border-line px-4 py-3 text-left text-base font-bold text-ink hover:border-kola hover:bg-kola-light hover:text-kola"><Icon size={21} className="text-kola" />{label}</button>; return id === "notifications" ? <NotificationBell key={id}>{item}</NotificationBell> : item; })}</nav>
