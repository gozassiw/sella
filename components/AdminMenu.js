    ...
    <>
      ...
      {open && <div className="fixed inset-0 z-[70] md:hidden" role="dialog" aria-modal="true" aria-label="Admin menu">
        ...
        <aside className="absolute right-0 top-0 h-full w-[min(88vw,360px)] overflow-y-auto bg-white p-5">
          ...
          <nav className="mt-5 space-y-1">{items.map(([id, label, Icon]) => <button key={id} type="button" onClick={() => jumpTo(id)} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-bold text-ink hover:bg-kola-light hover:text-kola"><Icon size={18} className="text-kola" />{label}</button>)}</nav>
