-- Sella Stage 27: pass the TransactPay provider fee to the funding customer.
-- Current provider pricing: 1.5% of the payment, capped at NGN 2,000.
-- The gross provider payment remains in the webhook event payload; only the
-- net amount is added to a buyer's wallet and wallet transaction history.
...
begin
  ...
  if v_buyer_wallet.id is not null then
    ...
    set balance = coalesce(v_buyer_wallet.balance, 0) + v_net_amount
    ...
    values (
      v_buyer_wallet.id,
      v_net_amount,
      'TransactPay wallet funding (1.5% fee deducted, capped at NGN 2,000)',
      p_payment_reference
    );

    return jsonb_build_object(
      'received', true,
      'credited', 'buyer_wallet',
      'wallet_id', v_buyer_wallet.id,
      'user_id', v_buyer_wallet.user_id,
      'amount', v_net_amount,
  ...
  if nullif(p_order_reference, '') is not null and p_order_reference ~* '^[0-9a-f]{8}-[0-9a-f-]{27}$' then
    select * into v_order
    from public.orders
    where id = p_order_reference::uuid
    limit 1
    for update;
  else
    select * into v_order
  ...
  set payment_status = 'paid',
      paid_at = now(),
      escrow_status = case when v_trusted then 'released' else 'held' end,
  ...
  return jsonb_build_object(
    'received', true,
    'credited', 'seller_wallet',
    'order_id', v_order.id,
    'user_id', v_store.owner_id,
    'amount', p_amount
  );
end;
$$;
