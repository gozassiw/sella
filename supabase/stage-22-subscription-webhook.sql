-- Sella Stage 22: confirmed TransactPay subscription payments.
...
declare v_subscription public.subscriptions%rowtype; v_store public.stores%rowtype; v_event_id uuid; v_owner_id uuid; v_days integer;
begin
  select * into v_subscription from public.subscriptions where (nullif(p_order_reference, '') is not null and id::text = p_order_reference) or (nullif(p_payment_reference, '') is not null and (payment_reference = p_payment_reference or id::text = p_payment_reference)) order by created_at desc limit 1;
