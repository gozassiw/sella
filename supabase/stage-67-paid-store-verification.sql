...
begin
  ...
  insert into public.store_verification_purchases(store_id, amount, paid_with, status, payment_reference) values (p_store_id, v_price, 'transfer', 'pending', null) returning * into v_purchase;
  return v_purchase;
end;
$$;
...
begin
  if auth.uid() is null then raise exception 'Not authorised'; end if;
  update public.store_verification_purchases v
  set payment_account_number = p_account_number, payment_account_name = p_account_name, payment_bank_name = p_bank_name, payment_reference = coalesce(p_payment_reference, v.payment_reference), payment_account_expires_at = now() + interval '30 minutes'
  from public.stores s
  where v.id = p_purchase_id and s.id = v.store_id and s.owner_id = auth.uid()
  returning v.* into v_purchase;
...
begin
  ...
  return jsonb_build_object(
    'stores', coalesce((select jsonb_agg((to_jsonb(s) - 'owner_id') || jsonb_build_object('seller_email',(select u.email from auth.users u where u.id=s.owner_id),'payout_accounts',coalesce((select jsonb_agg(to_jsonb(b)-'id'-'store_id' order by b.updated_at desc) from public.store_bank_accounts b where b.store_id=s.id),'[]'::jsonb)) order by s.created_at desc) from public.stores s),'[]'::jsonb),
    ...
    'verification_purchases', coalesce((select jsonb_agg(to_jsonb(v)||jsonb_build_object('stores',jsonb_build_object('name',s.name,'slug',s.slug)) order by v.created_at desc) from public.store_verification_purchases v left join public.stores s on s.id=v.store_id),'[]'::jsonb),
