-- Sella Stage 63: remove buyer deposit/transfer fees and apply plan-specific seller commissions.
-- Buyer funding and order transfers now credit/pay the exact amount sent.
-- Seller commission rates: trial/basic 3%, plus 2.8%, premium 2.5%.
...
as $$
  select case lower(coalesce(plan, 'trial'))
    when 'plus' then 2.8
    when 'biannual' then 2.8
    when 'premium' then 2.5
    when 'yearly' then 2.5
    else 3
  end
  from public.stores
  where id = p_store_id
...
revoke all on function public.seller_commission_rate(uuid) from public, anon, authenticated;

-- Checkout orders never add a buyer transfer surcharge.
...
begin
  ...
  v_payment_total := round(v_subtotal, 2);
...
-- Wallet payments use the seller's active plan commission.
...
begin
  ...
  where id = v_order.id;
  return jsonb_build_object('success', true, 'order_id', v_order.id, 'order_code', v_order.order_code, 'seller_net', v_net, 'commission', v_commission, 'commission_rate', v_commission_rate);
end;
$$;
grant execute on function public.pay_order_from_wallet(uuid) to authenticated;

-- TransactPay deposits credit the complete amount sent; order transfers also
-- require exactly the order amount because their surcharge is permanently zero.
...
begin
  ...
  if v_buyer_wallet.id is not null then
    ...
    values (v_buyer_wallet.id, v_net_amount, 'TransactPay wallet funding', p_payment_reference);
...
-- Public checkout settings expose only seller-side withdrawal information.
