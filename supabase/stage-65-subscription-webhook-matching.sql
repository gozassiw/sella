-- Sella Stage 65: robust TransactPay subscription matching.
-- Match plan payments by subscription ID, payment reference, provider account reference, or virtual account number.

update public.subscriptions
set payment_account_expires_at = started_at + interval '30 minutes'
where status = 'pending'
  and payment_account_number is not null
  and payment_account_expires_at is null;
...
-- Stage 52 intentionally restricts this money-moving function to the server role.
