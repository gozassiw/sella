-- Sella Stage 35: seller cost price and profit tracking.
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    insert into public.order_items(order_id, product_id, name, price, cost_price, quantity) values (v_order_id, v_product.id, v_product.name, v_product.price, v_product.cost_price, v_qty);
  end loop;
