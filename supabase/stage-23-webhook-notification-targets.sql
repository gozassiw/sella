-- Sella Stage 23: return notification recipients from payment processing.

create or replace function public.process_transactpay_webhook(
  p_event_key text, p_payload jsonb, p_successful boolean, p_account_number text default null, p_account_reference text default null, p_amount numeric default 0, p_payment_reference text default null, p_order_reference text default null
)
returns jsonb language plpgsql security definer set search_path = public as $$
declare v_buyer_wallet public.buyer_wallets%rowtype; v_order public.orders%rowtype; v_store public.stores%rowtype; v_wallet public.wallets%rowtype; v_commission_setting jsonb; v_threshold_setting jsonb; v_commission_rate numeric := 5; v_threshold integer := 3; v_commission numeric; v_net numeric; v_trusted boolean; v_event_id uuid;
begin
  ...
  if v_buyer_wallet.id is not null then
    update public.buyer_wallets set balance = coalesce(v_buyer_wallet.balance, 0) + p_amount where id = v_buyer_wallet.id;
    insert into public.buyer_wallet_transactions(buyer_wallet_id, amount, label, provider_reference) values (v_buyer_wallet.id, p_amount, 'TransactPay wallet funding', p_payment_reference);
    return jsonb_build_object('received', true, 'credited', 'buyer_wallet', 'wallet_id', v_buyer_wallet.id, 'user_id', v_buyer_wallet.user_id, 'amount', p_amount);
  end if;
  ...
  v_commission := round(p_amount * v_commission_rate / 100, 2); v_net := p_amount - v_commission; v_trusted := coalesce(v_store.trusted, false) or coalesce(v_store.completed_orders, 0) >= v_threshold;
  select * into v_wallet from public.wallets where store_id = v_order.store_id limit 1 for update;
  if v_wallet.id is null then insert into public.wallets(store_id, available, held) values (v_order.store_id, case when v_trusted then v_net else 0 end, case when v_trusted then 0 else v_net end) returning * into v_wallet; else update public.wallets set available = coalesce(v_wallet.available, 0) + case when v_trusted then v_net else 0 end, held = coalesce(v_wallet.held, 0) + case when v_trusted then 0 else v_net end where id = v_wallet.id returning * into v_wallet; end if;
  insert into public.wallet_transactions(wallet_id, order_id, kind, amount, note) values (v_wallet.id, v_order.id, case when v_trusted then 'credit' else 'hold' end, v_net, 'TransactPay payment');
  ...
  return jsonb_build_object('received', true, 'credited', 'seller_wallet', 'order_id', v_order.id, 'user_id', v_store.owner_id, 'amount', p_amount);
end;
$$;
