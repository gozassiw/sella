...
export default async function OnboardingPage() {
  ...
  const { data: store } = await supabase.from("stores").select("id,owner_id,name,slug,category,description,whatsapp,phone,address,legal_name,nin,cac_number,cac_file_url,passport_photo_url,approval_status").eq("owner_id", user.id).maybeSingle();
