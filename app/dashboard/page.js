...
export default async function DashboardHome() {
  ...
  return <div className="space-y-8">
    ...
    <section className="grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
      ...
      <div className="app-card p-5 sm:p-6"><div className="flex items-center justify-between"><div><p className="eyebrow text-kola">Store health</p><h2 className="mt-2 text-base font-extrabold">{trusted ? "Trusted seller" : "Build buyer trust"}</h2></div><span className="grid h-10 w-10 place-items-center rounded-2xl bg-kola-light text-kola"><ShieldCheck size={20} /></span></div><p className="mt-3 text-xs leading-5 text-muted">{trusted ? "Payments can move to your balance after confirmed delivery." : `${completedOrders} of 3 confirmed deliveries complete.`}</p>{!trusted && <div className="mt-5 h-2 overflow-hidden rounded-full bg-surface"><div className="h-full rounded-full bg-kola" style={{ width: `${Math.min(100, (completedOrders / 3) * 100)}%` }} /></div>}{trialDays !== null && <p className="mt-4 text-[11px] font-bold text-kola">{trialDays} days left in your 10-day trial · up to 15 products · <Link href="/dashboard/billing" className="underline">View plans</Link></p>}</div>
    </section>
