...
export default async function DashboardHome() {
  ...
  const [{ count: productCount }, { count: lowStockCount }, { count: openOrderCount }, { data: recentOrders }, { data: wallet }] = await Promise.all([
    ...
    supabase.from("orders").select("id,order_number,total,status,payment_status,created_at,customers(name)").eq("store_id", store.id).order("created_at", { ascending: false }).limit(5),
  ...
  const trusted = Boolean(store.trusted) || completedOrders >= 3;
  ...
  return <div className="space-y-8">
    ...
    {store.approval_status === "pending" && <div className="flex items-start gap-3 rounded-[20px] bg-amber-50 p-4 text-warning"><span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-warning" /><div><p className="text-sm font-extrabold">Your store is being reviewed</p><p className="mt-1 text-xs leading-5">Your 10-day trial starts after approval.</p></div></div>}
    ...
    <section className="grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
      <Link href="/dashboard/wallet" className="relative overflow-hidden rounded-[28px] bg-kola-dark p-6 text-white sm:p-7" style={{ boxShadow: "0 18px 45px rgba(9,84,58,.22)" }}>
        <div className="flex items-start justify-between"><div><p className="text-xs font-bold text-white/60">AVAILABLE TO WITHDRAW</p><p className="display mt-3 text-4xl sm:text-[42px]">{formatNaira(wallet?.available)}</p>{Number(wallet?.held || 0) > 0 && <p className="mt-2 text-xs text-white/65">{formatNaira(wallet.held)} held until delivery</p>}</div><span className="grid h-11 w-11 place-items-center rounded-2xl bg-white/10 text-mango"><WalletCards size={21} /></span></div>
      ...
      <div className="app-card p-5 sm:p-6"><div className="flex items-center justify-between"><div><p className="eyebrow text-kola">Store health</p><h2 className="mt-2 text-base font-extrabold">{trusted ? "Trusted seller" : "Build buyer trust"}</h2></div><span className="grid h-10 w-10 place-items-center rounded-2xl bg-kola-light text-kola"><ShieldCheck size={20} /></span></div><p className="mt-3 text-xs leading-5 text-muted">{trusted ? "Payments can move to your balance after confirmed delivery." : `${completedOrders} of 3 confirmed deliveries complete.`}</p>{!trusted && <div className="mt-5 h-2 overflow-hidden rounded-full bg-surface"><div className="h-full rounded-full bg-kola" style={{ width: `${Math.min(100, (completedOrders / 3) * 100)}%` }} /></div>}{trialDays !== null && <p className="mt-4 text-[11px] font-bold text-kola">{trialDays} days left in your 10-day trial · <Link href="/dashboard/billing" className="underline">View plans</Link></p>}</div>
    </section>
    ...
    <section className="grid gap-5 lg:grid-cols-[1.25fr_.75fr]">
      <div><div className="flex items-center justify-between"><div><p className="eyebrow text-kola">Orders</p><h2 className="display mt-2 text-2xl">Recent activity</h2></div><Link href="/dashboard/orders" className="text-xs font-bold text-kola">See all</Link></div><div className="app-card mt-4 divide-y divide-line overflow-hidden">{recentOrders?.length ? recentOrders.map((order) => <Link key={order.id} href={`/dashboard/orders?order=${order.id}`} className="flex items-center gap-3 p-4"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-kola-light text-kola"><Package size={18} /></span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">#{order.order_number} · {order.customers?.name || "Customer"}</p><p className="mt-1 text-xs text-muted">{order.payment_status}</p></div><div className="text-right"><p className="text-sm font-extrabold">{formatNaira(order.total)}</p><div className="mt-1"><StatusPill status={order.status} /></div></div><ChevronRight size={16} className="text-muted" /></Link>) : <div className="flex flex-col items-center px-5 py-10 text-center"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-kola-light text-kola"><ShoppingBag size={21} /></span><p className="mt-4 text-sm font-extrabold">No orders yet</p><p className="mt-2 max-w-xs text-xs leading-5 text-muted">Share your store link so buyers can find your products.</p><Link href={storeUrl(SITE_URL, store.slug)} target="_blank" className="btn-soft mt-4">Open storefront</Link></div>}</div></div>
