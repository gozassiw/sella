...
export default async function DashboardHome() {
  ...
  return <div className="space-y-8">
    ...
    <section className="grid gap-5 lg:grid-cols-[1.25fr_.75fr]">
      <div><div className="flex items-center justify-between"><div><p className="eyebrow text-kola">Orders</p><h2 className="display mt-2 text-2xl">Recent activity</h2></div><Link href="/dashboard/orders" className="text-xs font-bold text-kola">See all</Link></div><div className="app-card mt-4 divide-y divide-line overflow-hidden">{recentOrders?.length ? recentOrders.map((order) => <Link key={order.id} href={`/dashboard/orders?order=${order.id}`} className="flex items-center gap-3 p-4"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-kola-light text-kola"><Package size={18} /></span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">#{order.order_code} · {order.customers?.name || "Customer"}</p><p className="mt-1 text-xs text-muted">{order.payment_status}</p></div><div className="text-right"><p className="text-sm font-extrabold">{formatNaira(order.total)}</p><div className="mt-1"><StatusPill status={order.status} /></div></div><ChevronRight size={16} className="text-muted" /></Link>) : <div className="flex flex-col items-center px-5 py-10 text-center"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-kola-light text-kola"><ShoppingBag size={21} /></span><p className="mt-4 text-sm font-extrabold">No orders yet</p><p className="mt-2 max-w-xs text-xs leading-5 text-muted">Share your store link so buyers can find your products.</p><Link href={storeUrl(SITE_URL, store.slug)} target="_blank" className="btn-soft mt-4">Open storefront</Link></div>}</div></div>
