...
export default async function DashboardLayout({ children }) {
  ...
  return <div className="min-h-screen bg-surface md:flex"><LiveWorkspaceRefresh scope="seller" storeId={store.id} ownerId={store.owner_id} /><DashboardNav ownerId={store.owner_id} store={{ name: store.name, slug: store.slug, seller_code: store.seller_code, logo_url: store.logo_url, approval_status: store.approval_status }} /><main className="min-w-0 flex-1 pb-28 md:pb-0 md:pt-20"><div className="mx-auto max-w-[1180px] px-4 py-6 sm:px-6 sm:py-8 lg:px-10 lg:py-10">{blocked ? <AccessNotice held={held} store={store} /> : <>{children}</>}</div></main>{!blocked && !store.commission_acknowledged_at && <SellerCommissionAcknowledgement />}</div>;
}
