...
export default async function BillingPage() {
  ...
  const [{ data: wallet }, { data: subscriptions }] = await Promise.all([
    ...
    supabase.from("subscriptions").select("plan,amount,paid_with,started_at,expires_at,status,payment_reference,paid_at,created_at").eq("store_id", store.id).order("started_at", { ascending: false }).limit(20),
  ]);
  ...
  return <div className="space-y-6">
    ...
    <section className="panel"><div className="flex items-center justify-between gap-4"><h2 className="font-semibold">Subscription history</h2><span className="chip">{subscriptions?.length || 0} records</span></div><div className="mt-4 space-y-3">{subscriptions?.length ? subscriptions.map((item, index) => <div key={`${item.plan}-${item.started_at || index}`} className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-line p-4"><div><p className="text-sm font-extrabold">{planNames[item.plan] || item.plan} <span className="font-normal text-muted">· {item.paid_with || "transfer"}</span></p><p className="mt-1 text-xs text-muted">{item.paid_at ? `Paid ${new Date(item.paid_at).toLocaleDateString("en-NG")}` : `Started ${new Date(item.started_at || item.created_at).toLocaleDateString("en-NG")}`}{item.expires_at ? ` · Ends ${new Date(item.expires_at).toLocaleDateString("en-NG")}` : ""}</p></div><StatusBadge status={item.status} /></div>) : <p className="text-sm text-muted">No subscription payments yet.</p>}</div></section>
  </div>;
}
