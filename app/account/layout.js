...
export default async function AccountLayout({ children }) {
  ...
  return <div className="min-h-screen bg-surface pb-28"><header className="sticky top-0 z-40 border-b border-line bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-[1120px] items-center justify-between gap-4 px-4 py-3 sm:px-6"><SellaBrand href="/account" compact /><div className="flex items-center gap-2"><Link href="/account/notifications" aria-label="Notifications" className="grid h-10 w-10 place-items-center rounded-full bg-surface text-ink"><Bell size={18} /></Link><Link href="/account#discover" aria-label="Search stores" className="grid h-10 w-10 place-items-center rounded-full bg-surface text-ink"><Search size={18} /></Link><Link href="/cart" aria-label="Cart" className="grid h-10 w-10 place-items-center rounded-full bg-kola text-white"><ShoppingBag size={17} /></Link></div></div></header><main className="mx-auto max-w-[1120px] px-4 py-6 sm:px-6 lg:px-8">{children}</main><BuyerBottomNav /></div>;
}
