...
export default async function BuyerProfilePage() {
  ...
  return <div className="mx-auto max-w-3xl space-y-8">
    ...
    <section><div className="flex items-center justify-between"><div><p className="eyebrow text-kola">Following</p><h2 className="display mt-2 text-2xl">Your stores</h2></div><span className="chip"><Heart size={13} className="mr-1.5" />{follows?.length || 0}</span></div><div className="mt-4 grid gap-3 sm:grid-cols-2">{follows?.length ? follows.map((follow) => <div key={follow.store_id} className="app-card flex items-center gap-3 p-4"><Link href={`/s/${follow.stores.slug}`} className="flex min-w-0 flex-1 items-center gap-3">{follow.stores.logo_url ? <Image src={follow.stores.logo_url} alt="" width={48} height={48} className="h-12 w-12 rounded-[17px] object-cover" /> : <span className="grid h-12 w-12 place-items-center rounded-[17px] bg-kola text-sm font-extrabold text-white">{follow.stores.name.charAt(0)}</span>}<span className="min-w-0 flex-1"><span className="block truncate text-sm font-extrabold">{follow.stores.name}</span><span className="mt-1 block truncate text-xs text-muted">{follow.stores.category || "Independent store"}</span></span><ChevronRight size={16} className="text-muted" /></Link><UnfollowStoreButton storeId={follow.store_id} /></div>) : <div className="app-card p-8 text-center sm:col-span-2"><Heart size={22} className="mx-auto text-kola" /><p className="mt-4 text-sm font-extrabold">No followed stores yet</p><p className="mt-2 text-xs text-muted">Follow a seller to keep their newest products on your Home feed.</p><Link href="/account#discover" className="btn-soft mt-5">Discover stores</Link></div>}</div></section>
  </div>;
}
