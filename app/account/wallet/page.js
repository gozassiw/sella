...
export default async function WalletPage() {
  ...
  let transactions = [];
  let paymentSettings = { deposit_fee_rate: 1.5 };
  const { data: settings } = await supabase.rpc("get_public_payment_settings");
  if (settings) paymentSettings = settings;
  if (wallet?.id) {
  ...
  const depositRate = Number(paymentSettings.deposit_fee_rate ?? 1.5);
