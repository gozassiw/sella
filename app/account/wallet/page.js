...
export default async function WalletPage() {
  ...
  const { data: transactions } = wallet
    ? await supabase.from("buyer_wallet_transactions").select("id,amount,label,provider_reference,created_at,order_id,orders(order_code,stores(name))").eq("buyer_wallet_id", wallet.id).order("created_at", { ascending: false }).limit(12)
    : { data: [] };
  ...
  return <div className="mx-auto max-w-3xl space-y-8">
    ...
    <section><div className="flex items-center justify-between"><h2 className="text-base font-extrabold">Recent activity</h2><span className="text-xs font-bold text-muted">Latest 12</span></div><div className="app-card mt-4 divide-y divide-line overflow-hidden">{transactions?.length ? transactions.map((txn) => {
      const amount = Number(txn.amount || 0);
      const debit = amount < 0;
      const order = txn.orders;
      const detail = order ? `Order #${order.order_code}${order.stores?.name ? ` · ${order.stores.name}` : ""}` : "TransactPay deposit";
      const row = <div className="flex items-center gap-3 p-4"><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-2xl ${debit ? "bg-surface text-ink" : "bg-kola-light text-kola"}`}>{debit ? <ArrowUpRight size={18} /> : <ArrowDownLeft size={18} />}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{debit ? "Order payment" : "TransactPay deposit"}</p><p className="mt-1 truncate text-[11px] text-muted">{detail} · {new Date(txn.created_at).toLocaleString()}</p></div><p className={`text-sm font-extrabold ${debit ? "text-ink" : "text-success"}`}>{debit ? "-" : "+"}{formatNaira(Math.abs(amount))}</p></div>;
      return order?.order_code ? <Link key={txn.id} href={`/account/orders/${txn.order_id}`} className="block hover:bg-surface">{row}</Link> : <div key={txn.id}>{row}</div>;
    }) : <p className="p-8 text-center text-sm text-muted">No wallet activity yet.</p>}</div></section>
  </div>;
}
